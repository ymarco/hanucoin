...
def handleInSoc(sock,addr):
	global blocks_got_updated
	try:
		in_msg = ""
		watchdog = currentTime
		while True:
			data = sock.recv(1<<10) #KiloByte	
			if not data or currentTime- watchdog >5: break #stop when receiving data, or when 5 secs passed
			in_msg += data 	
		...
		bytes_sent = 0
		watchdog = currentTime
		while bytes_sent<len(out_message) and currentTime-watchdog <5: #stop when finished sending or 5 secs passed
			bytes_sent += sock.send(out_message[bytes_sent:])
		print Fore.GREEN + "[handleInSoc]: sent %d bytes back to %s" % (bytes_sent,strAddress(addr))
		sock.shutdown(2)
...
def inputLoop():
	...
	while True:
		...
		handleInSocThread = threading.Thread(target = handleInSoc(sock,addr), name = "handleInSoc")
