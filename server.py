...
def createMsg(cmd, node_list, block_list):
	parsed_cmd = struct.pack(">I", cmd)
	nodes_count = struct.pack(">I", len(node_list))
...
def handleInSock(sock, addr):
	try:
		nodes, blocks = recvMsg(sock, desired_cmd=1)
		updateByNodes(nodes)
...
def CommMain():  # Send and receive packets from Tal
	...
	try:
		...
		nodes,blocks=recvMsg(sock, desired_cmd=2)
		updateByNodes(nodes)
		updateByBlocks(blocks)
	...
	if nodes_got_updated or blocks_got_updated or currentTime - TIME_BETWEEN_SENDS >= sendBuffer:  # Every 5 minutes, or when nodes_got_updated is true:
		...
		for nod in random.sample(activeNodes.viewvalues(), min(3, len(activeNodes))):  # Random 3 addresses (or less when there are less than 3 available)
			...
			try:
				...
				nodes,blocks=recvMsg(out_socket, desired_cmd=2)
				updateByNodes(nodes)
